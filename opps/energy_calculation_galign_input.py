...
def energy_calc(input_mols, templ_file_path, tt):
    # Set random seeds and device
    scores = []
    tf = tt + templ_file_path[-8:-5]
    input_mols = [Chem.MolToSmiles(i) for i in input_mols]
    for ii, i in enumerate(input_mols):
        with open(f'smi/{ii}_{tf}_f2.smi','w') as f:
            f.write(i)
        os.chdir('/home/hakjean/galaxy2/developments/MolGen/CSearch_revised/smi/')
        os.system(f"obabel -ismi {ii}_{tf}_f2.smi -omol2 -O {ii}_{tf}_f2.mol2 --gen3D")
        if not os.path.exists(f"{ii}_{tf}_f2.mol2"):
            smi = next(nuri.readstring("smi",i))
            with open(f"{ii}_{tf}_f2.mol2", 'w') as gt:
                gt.write(nuri.to_mol2(smi))
        
        #os.system(f"/applic/corina/corina -i t=sdf {i}_f.sdf -o t=mol2 {i}_f.mol2")
        ff = str(subprocess.run(["galign",f"{ii}_{tf}_f2.mol2",f"{templ_file_path}"],capture_output=True)).split()
        print(ff)
        ...
        os.system(f"rm {ii}_{tf}_f2.smi")
        #os.sytem(f"rm *.mol2")
        ...
        #    scores.append(0)
        #    continue
        os.system(f'rm {ii}_{tf}_f2.mol2')
