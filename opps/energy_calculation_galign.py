...
def energy_calc(input_mols, input_file_path):
    ...
    for ii, i in enumerate(input_mols):
        ...
        if not os.path.exists(f"{ii}_f.mol2"):
            smi = nuri.readstring("smi",i)
            with open(f"{ii}_f.mol2", 'w') as gt:
                gt.write(nuri.to_mol2(smi))
        
        #os.system(f"/applic/corina/corina -i t=sdf {i}_f.sdf -o t=mol2 {i}_f.mol2")
        ff = str(subprocess.run(["galign",f"{ii}_f.mol2","/home/hakjean/galaxy2/developments/MolGen/CSearch_revised/data/3TI5_ori_1.mol2"],capture_output=True)).split()
        print(ff)
