...
def make_fp_array(smi_list,fpty):
    for i in smi_list:
        mol = str(i)
        if mol.find("~") != -1:
            mol = mol.replace("~","")
        mol = pybel.readstring("smi",mol)
        fp = mol.calcfp(fptype=fpty)
        a = str(fp)
        b = a.split(',')
...
def main(args):
    ...
    compl = make_fp_array(bank_smiles,args.fp)
    n = len(compl)
...
if __name__=="__main__":
    ...
    parser.add_argument("-f", "--fp", type=str, default='fp4', help="Fingerprint Type for distance")
