...
class CSA(object):
    def __init__(self, filter_lipinski=False, use_ML=True, ref_lig=None):
        ...
        self.n_csa_iter=1
        self.n_seed_cycle=1
        self.max_opt_cycle = 1000
        self.dist_mat = np.zeros((self.n_bank, self.n_bank))
        self.use_ML=use_ML

        self.seed_cycle = 1

        self.n_opt_to_D_min = 1000
    ...
    def initialize_csa(self, job, prot_fn, init_bank_mol2_fn, cntr_R, n_proc=None):
        ...
        #initial bank
        self.read_initial_bank(init_bank_mol2_fn)
    ...
    def run(self):
        ...
        for i_csa_iter in range(self.n_csa_iter):
            for i_opt_cycle in range(1,self.max_opt_cycle+1):
                self.select_seeds()
                new_mol_s, new_energy_s = self.make_new_confs(i_opt_cycle)
                self.update_bank(new_mol_s, new_energy_s)
                self.update_distance()
                ...
                if len(self.seed_mask) ==0:
                    return
    def read_initial_bank(self, mol2_fn):
        i_cycle = 0
        self.bank_pool = []
        #Read Molecule
        i_mol = 0
    ...
    def setup_distance(self, new_mol_s):
        nst = float(self.n_opt_to_D_min)/len(new_mol_s)

        n_mol = len(new_mol_s)
        dist_mat = np.zeros((n_mol,n_mol))
    ...
    def make_new_confs(self, i_cycle):
        ...
        for i_seed in seed_selected:
            ...
            gen_RDKmol_s = gen_crossover(seed_mol, partner_mol, filters=self.catalog_filters,\
        ...
        for i_seed in seed_selected:
            ...
            for reaction in self.reaction_s:
                ...
                products = reaction.run_reaction(reactants, filters=self.catalog_filters,\
                        filter_lipinski=self.filter_lipinski)
    ...
    def update_bank(self, new_mol_s, new_energy_s):
        i_Emax_bank_u = np.argsort(self.energy_s)[-1]

        for i_new in range(len(new_mol_s)):
            i_energy = new_energy_s[i_new]
            if i_energy >= self.energy_s[i_Emax_bank_u]:
                continue
            i_mol = new_mol_s[i_new]
            # check closest bank member
            dist_s = [calc_tanimoto_distance(bank_mol, i_mol) for bank_mol in self.bank_pool]
            ...
            if (min_dist < self.Dcut):
                if i_energy < self.energy_s[min_idx]:
                    print('%d %.3f was replaced to %d %.3f in same group'%(min_idx,\
                            self.energy_s[min_idx], i_new, i_energy))
                    self.bank_pool[min_idx] = i_mol
                    self.energy_s[min_idx] = i_energy
                    if not min_idx in self.seed_mask:
                        self.seed_mask.append(min_idx)
            else:
                print('%d %.3f was replaced to %d %.3f in new group'%(i_Emax_bank_u,\
                        self.energy_s[i_Emax_bank_u], i_new, i_energy))
                self.bank_pool[i_Emax_bank_u] = i_mol
                self.energy_s[i_Emax_bank_u] = i_energy
                if not i_Emax_bank_u in self.seed_mask:
                    self.seed_mask.append(i_Emax_bank_u)
            i_Emax_bank_u = np.argsort(self.energy_s)[-1]
    def print_log(self):
        for i in range(self.n_bank):
            print('Bank %d:%.3f'%(i+1, self.energy_s[i]))
...
if __name__=='__main__':
    if rank==0:
        ...
        mol2_fn = Galaxy.core.FilePath(sys.argv[2])
        opt_fn =  sys.argv[3]
